...
import CustomersSales from './CustomersSales';
...
export const PdfCard: React.FC<PdfCardProps> = ({ title }) => {
    ...
    const handleDownload = async () => {
        ...
        console.log(prometPoKupcima);
        if (prometPoKupcima) {
            const blob = await pdf(<CustomersSales prometPoKupcima={prometPoKupcima} />).toBlob();
            saveAs(blob, 'CustomersSales.pdf');
            console.log(blob);
        }
        else {
            console.error('No customers sales available to download.');
        }
