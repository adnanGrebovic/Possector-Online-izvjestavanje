...
export const PdfCard: React.FC<PdfCardProps> = ({ title }) => {
    ...
    const handleDownload = async () => {
        ...
        console.log(stanjeRobe);
        if (stanjeRobe) {
            const blob = await pdf(<StorageReport stanjeRobe={stanjeRobe} />).toBlob();
            saveAs(blob, 'StorageReport.pdf');
            console.log(blob);
        }
        else {
            console.error('No storage report available to download.');
        }
