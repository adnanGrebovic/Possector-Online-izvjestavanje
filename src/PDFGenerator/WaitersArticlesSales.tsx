...
interface InvoiceProps {
    prodajaArtikalaPoOperaterima: UniverzalniItems;
  }

export const ArticlesSales: React.FC<InvoiceProps> = ({prodajaArtikalaPoOperaterima}) => {
    ...
    const InvoiceTitle = () => (
        <View style={styles.titleContainer}>
                <View style={styles.spaceBetween}>
                    <Text style={styles.reportTitle}>PRODAJA ARTIKALA PO OPERATERIMA</Text>
                </View>
        </View>
    );
    ...
    const TableBody = () => (
        prodajaArtikalaPoOperaterima.Items.map((save: Items, index)=>(
            <Fragment key={index}>
    ...
    const TableTotal = () => (
        <View style={{ width:'100%', flexDirection :'row'}}>
                ...
                <View style={styles.tbody}>
                    <Text>
                    {prodajaArtikalaPoOperaterima.Items.reduce((acc, item) => acc + parseFloat(item.TotalFormatted), 0)}
                    </Text>  
                </View>
            </View>
    );
