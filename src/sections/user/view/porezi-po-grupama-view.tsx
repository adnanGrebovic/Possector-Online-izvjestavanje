...
import PdfCard from 'src/PDFGenerator/PDFCardTaxesTags';
...
export function PoreziPoGrupamaView() {
  const { poreziPoGrupama, loading } = useAppSelector(state => state.taxes_tags);
  ...
  useEffect(()=>{
    if(poreziPoGrupama!==null){
      setFetchedData(poreziPoGrupama.Items);
    }
  }, [poreziPoGrupama])
  ...
  return (
    <DashboardContent>
      ...
      <Box sx={{position: 'relative', left: '700px', bottom: '170px'}}>
        <PdfCard title='Porezi po grupama'/>
      </Box>
                ...
                {paginatedData && paginatedData.length > 0 ? (
                  paginatedData.map((item: Items, index) => (
                    <TableRow key={index}>
                      ...
                      <TableCell sx={{ paddingLeft: '30px', textAlign: 'left', minWidth: 150, position: 'relative', left: '100px' }}>
                        {item.Name}
                      </TableCell>
