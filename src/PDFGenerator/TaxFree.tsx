...
interface InvoiceProps {
    oslobodjenoPoreza: UniverzalniItems;
  }

export const ArticlesSales: React.FC<InvoiceProps> = ({oslobodjenoPoreza}) => {
    ...
    const InvoiceTitle = () => (
        <View style={styles.titleContainer}>
                <View style={styles.spaceBetween}>
                    <Text style={styles.reportTitle}>OSLOBODJENO POREZA</Text>
                </View>
        </View>
    );
    ...
    const TableBody = () => (
        oslobodjenoPoreza.Items.map((save: Items, index)=>(
            <Fragment key={index}>
    ...
    const TableTotal = () => (
        <View style={{ width:'100%', flexDirection :'row'}}>
                ...
                <View style={styles.tbody}>
                    <Text>
                    {oslobodjenoPoreza.Items.reduce((acc, item) => acc + parseFloat(item.TotalFormatted), 0)}
                    </Text>  
                </View>
            </View>
    );
