...
interface InvoiceProps {
    prometPoKupcima: UniverzalniItems;
  }

export const ArticlesSales: React.FC<InvoiceProps> = ({prometPoKupcima}) => {
    ...
    const InvoiceTitle = () => (
        <View style={styles.titleContainer}>
                <View style={styles.spaceBetween}>
                    <Text style={styles.reportTitle}>PROMET PO KUPCIMA</Text>
                </View>
        </View>
    );
    ...
    const TableBody = () => (
        prometPoKupcima.Items.map((save: Items, index)=>(
            <Fragment key={index}>
    ...
    const TableTotal = () => (
        <View style={{ width:'100%', flexDirection :'row'}}>
                ...
                <View style={styles.tbody}>
                    <Text>
                    {prometPoKupcima.Items.reduce((acc, item) => acc + parseFloat(item.TotalFormatted), 0)}
                    </Text>  
                </View>
            </View>
    );
