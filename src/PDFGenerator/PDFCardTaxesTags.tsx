...
export const PdfCard: React.FC<PdfCardProps> = ({ title }) => {
    ...
    const handleDownload = async () => {
        ...
        console.log(poreziPoGrupama);
        if (poreziPoGrupama) {
            const blob = await pdf(<TaxesTags poreziPoGrupama={poreziPoGrupama} />).toBlob();
            saveAs(blob, 'TaxesTags.pdf');
            console.log(blob);
        }
        else {
            console.error('No taxes tags available to download.');
        }
