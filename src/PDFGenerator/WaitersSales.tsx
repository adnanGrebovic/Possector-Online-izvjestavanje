...
interface InvoiceProps {
    prodajaPoOperaterima: UniverzalniItems;
  }

export const ArticlesSales: React.FC<InvoiceProps> = ({prodajaPoOperaterima}) => {
    ...
    const InvoiceTitle = () => (
        <View style={styles.titleContainer}>
                <View style={styles.spaceBetween}>
                    <Text style={styles.reportTitle}>PRODAJA PO OPERATERIMA</Text>
                </View>
        </View>
    );
    ...
    const TableBody = () => (
        prodajaPoOperaterima.Items.map((save: Items, index)=>(
            <Fragment key={index}>
    ...
    const TableTotal = () => (
        <View style={{ width:'100%', flexDirection :'row'}}>
                ...
                <View style={styles.tbody}>
                    <Text>
                    {prodajaPoOperaterima.Items.reduce((acc, item) => acc + parseFloat(item.TotalFormatted), 0)}
                    </Text>  
                </View>
            </View>
    );
