...
import { TableRow, TableCell, TableHead, TableFooter } from '@mui/material';
import PdfCard from 'src/PDFGenerator/PDFCardTaxesTotal';
...
export function PoreziView() {
  const { porezi, loading } = useAppSelector(state => state.taxes_total);
  ...
  return (
    <DashboardContent>
      <Box display='flex' alignContent='center' mb={5}>
                ...
                {paginatedData && paginatedData.length > 0 ? (
                  paginatedData.map((item: Items, index) => (
                    <TableRow key={index}>
                      ...
                      <TableCell sx={{ paddingRight: '40px', textAlign: 'right', minWidth: 100, position: 'relative', left: '105px' }}>
                        {item.Value}
                      </TableCell>
                      ...
                      <TableCell sx={{ paddingRight: '40px', textAlign: 'right', minWidth: 150, position: 'relative', left: '120px' }}>
                        {item.TotalFormatted}
                      </TableCell>
