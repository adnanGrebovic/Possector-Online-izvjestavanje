...
import PdfCard from 'src/PDFGenerator/PDFCardCustomersSales';
...
export function PrometPoKupcimaView() {
  const { prometPoKupcima, loading } = useAppSelector(state => state.customers_sales);
  ...
  useEffect(()=>{
    if(prometPoKupcima!==null){
      setFetchedData(prometPoKupcima.Items);
    }
  }, [prometPoKupcima])
  ...
  return (
    <DashboardContent>
      ...
      <Box sx={{position: 'relative', left: '700px', bottom: '170px'}}>
        <PdfCard title='Promet po kupcima'/>
      </Box>
                ...
                {paginatedData && paginatedData.length > 0 ? (
                  paginatedData.map((item: Items, index) => (
                    <TableRow key={index}>
                      ...
                      <TableCell sx={{ paddingLeft: '30px', textAlign: 'left', minWidth: 150, position: 'relative', left: '85px' }}>
                        {item.Name}
                      </TableCell>
                      ...
                      <TableCell sx={{ paddingRight: '40px', textAlign: 'right', minWidth: 150, position: 'relative', left: '85px' }}>
                        {item.TotalFormatted}
                      </TableCell>
