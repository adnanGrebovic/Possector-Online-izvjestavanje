...
import WaitersSales from './WaitersSales';
...
export const PdfCard: React.FC<PdfCardProps> = ({ title }) => {
    ...
    const { prodajaPoOperaterima, loading } = useAppSelector((state) => state.waiters_sales);


    const handleDownload = async () => {

        console.log(prodajaPoOperaterima);
        if (prodajaPoOperaterima) {
            const blob = await pdf(<WaitersSales prodajaPoOperaterima={prodajaPoOperaterima} />).toBlob();
            saveAs(blob, 'WaitersSales.pdf');
            console.log(blob);
        }
        else {
            console.error('No waiters sales available to download.');
        }
