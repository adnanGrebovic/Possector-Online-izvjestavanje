...
interface InvoiceProps {
    stanjeRobe: UniverzalniItems;
  }

export const ArticlesSales: React.FC<InvoiceProps> = ({stanjeRobe}) => {
    ...
    const InvoiceTitle = () => (
        <View style={styles.titleContainer}>
                <View style={styles.spaceBetween}>
                    <Text style={styles.reportTitle}>STANJE ROBE</Text>
                </View>
        </View>
    );
    ...
    const TableBody = () => (
        stanjeRobe.Items.map((save: Items, index)=>(
            <Fragment key={index}>
    ...
    const TableTotal = () => (
        <View style={{ width:'100%', flexDirection :'row'}}>
                ...
                <View style={styles.tbody}>
                    <Text>
                    {stanjeRobe.Items.reduce((acc, item) => acc + parseFloat(item.TotalFormatted), 0)}
                    </Text>  
                </View>
            </View>
    );
