...
interface InvoiceProps {
    vrstePlacanja: UniverzalniItems;
  }

export const ArticlesSales: React.FC<InvoiceProps> = ({vrstePlacanja}) => {
    ...
    const InvoiceTitle = () => (
        <View style={styles.titleContainer}>
                <View style={styles.spaceBetween}>
                    <Text style={styles.reportTitle}>VRSTE PLACANJA</Text>
                </View>
        </View>
    );
    ...
    const TableBody = () => (
        vrstePlacanja.Items.map((save: Items, index)=>(
            <Fragment key={index}>
    ...
    const TableTotal = () => (
        <View style={{ width:'100%', flexDirection :'row'}}>
                ...
                <View style={styles.tbody}>
                    <Text>
                    {vrstePlacanja.Items.reduce((acc, item) => acc + parseFloat(item.TotalFormatted), 0)}
                    </Text>  
                </View>
            </View>
    );
