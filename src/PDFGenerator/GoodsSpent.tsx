...
interface InvoiceProps {
    utrosenaRoba: UniverzalniItems;
  }

export const ArticlesSales: React.FC<InvoiceProps> = ({utrosenaRoba}) => {
    ...
    const InvoiceTitle = () => (
        <View style={styles.titleContainer}>
                <View style={styles.spaceBetween}>
                    <Text style={styles.reportTitle}>UTROSENA ROBA</Text>
                </View>
        </View>
    );
    ...
    const TableBody = () => (
        utrosenaRoba.Items.map((save: Items, index)=>(
            <Fragment key={index}>
    ...
    const TableTotal = () => (
        <View style={{ width:'100%', flexDirection :'row'}}>
                ...
                <View style={styles.tbody}>
                    <Text>
                    {utrosenaRoba.Items.reduce((acc, item) => acc + parseFloat(item.TotalFormatted), 0)}
                    </Text>  
                </View>
            </View>
    );
