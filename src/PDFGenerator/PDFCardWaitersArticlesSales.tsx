...
import WaitersArticlesSales from './WaitersArticlesSales';
...
export const PdfCard: React.FC<PdfCardProps> = ({ title }) => {
    ...
    const handleDownload = async () => {
        ...
        console.log(prodajaArtikalaPoOperaterima);
        if (prodajaArtikalaPoOperaterima) {
            const blob = await pdf(<WaitersArticlesSales prodajaArtikalaPoOperaterima={prodajaArtikalaPoOperaterima} />).toBlob();
            saveAs(blob, 'WaitersArticlesSales.pdf');
            console.log(blob);
        }
        else {
            console.error('No waiters articles sales available to download.');
        }
