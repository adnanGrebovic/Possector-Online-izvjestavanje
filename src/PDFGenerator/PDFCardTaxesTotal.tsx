...
import TaxesTotal from './TaxesTotal';
...
export const PdfCard: React.FC<PdfCardProps> = ({ title }) => {
    ...
    const handleDownload = async () => {
        ...
        console.log(porezi);
        if (porezi) {
            const blob = await pdf(<TaxesTotal porezi={porezi} />).toBlob();
            saveAs(blob, 'TaxesTotal.pdf');
            console.log(blob);
        }
        else {
            console.error('No total taxes available to download.');
        }
