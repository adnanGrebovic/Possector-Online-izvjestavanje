...
interface InvoiceProps {
    porezi: UniverzalniItems;
  }

export const ArticlesSales: React.FC<InvoiceProps> = ({porezi}) => {
    ...
    const InvoiceTitle = () => (
        <View style={styles.titleContainer}>
                <View style={styles.spaceBetween}>
                    <Text style={styles.reportTitle}>POREZI</Text>
                </View>
        </View>
    );
    ...
    const TableBody = () => (
        porezi.Items.map((save: Items, index)=>(
            <Fragment key={index}>
    ...
    const TableTotal = () => (
        <View style={{ width:'100%', flexDirection :'row'}}>
                ...
                <View style={styles.tbody}>
                    <Text>
                    {porezi.Items.reduce((acc, item) => acc + parseFloat(item.TotalFormatted), 0)}
                    </Text>  
                </View>
            </View>
    );
