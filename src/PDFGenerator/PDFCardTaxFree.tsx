...
export const PdfCard: React.FC<PdfCardProps> = ({ title }) => {
    ...
    const handleDownload = async () => {
        ...
        console.log(oslobodjenoPoreza);
        if (oslobodjenoPoreza) {
            const blob = await pdf(<TaxFree oslobodjenoPoreza={oslobodjenoPoreza} />).toBlob();
            saveAs(blob, 'TaxFree.pdf');
            console.log(blob);
        }
        else {
            console.error('No tax free info available to download.');
        }
