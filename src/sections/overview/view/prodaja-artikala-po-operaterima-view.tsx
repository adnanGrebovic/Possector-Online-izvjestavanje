...
import PdfCard from 'src/PDFGenerator/PDFCardWaitersArticlesSales';
...
export function ProdajaArtikalaPoOperaterimaView() {
  const { prodajaArtikalaPoOperaterima, loading } = useAppSelector(state => state.waiters_articles_sales);
  ...
  useEffect(()=>{
    if(prodajaArtikalaPoOperaterima!==null){
      setFetchedData(prodajaArtikalaPoOperaterima.Items);
    }
  }, [prodajaArtikalaPoOperaterima])
                ...
                {paginatedData && paginatedData.length > 0 ? (
                  paginatedData.map((item: Items, index) => (
                    <TableRow key={index}>
                      ...
                      <TableCell sx={{ paddingRight: '40px', textAlign: 'right', minWidth: 100, position: 'relative', left: '80px' }}>
                        {item.Value}
                      </TableCell>
                      ...
                      <TableCell sx={{ paddingRight: '40px', textAlign: 'right', minWidth: 150, position: 'relative', left: '105px' }}>
                        {item.TotalFormatted}
                      </TableCell>
