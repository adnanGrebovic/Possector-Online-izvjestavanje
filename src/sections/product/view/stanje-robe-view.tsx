...
import PdfCard from 'src/PDFGenerator/PDFCardStorageReport';
...
export function StanjeRobeView() {
  const { stanjeRobe, loading } = useAppSelector(state => state.storage_report);
  ...
  useEffect(()=>{
    if(stanjeRobe!==null){
      setFetchedData(stanjeRobe.Items);
    }
  }, [stanjeRobe])
                ...
                {paginatedData && paginatedData.length > 0 ? (
                  paginatedData.map((item: Items, index) => (
                    <TableRow key={index}>
                      ...
                      <TableCell  sx={{ paddingLeft: '40px', textAlign: 'right', minWidth: 150, position: 'relative', left: '85px' }}>
                        {item.TotalFormatted}
                      </TableCell>
