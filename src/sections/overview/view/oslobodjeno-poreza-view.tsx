...
import PdfCard from 'src/PDFGenerator/PDFCardTaxFree';
...
export function OslobodjenoPorezaView() {
  const { oslobodjenoPoreza, loading } = useAppSelector(state => state.tax_free);
  ...
  useEffect(()=>{
    if(oslobodjenoPoreza!==null){
      setFetchedData(oslobodjenoPoreza.Items);
    }
  }, [oslobodjenoPoreza])
  ...
  return (
    <DashboardContent>
      ...
      <Box sx={{position: 'relative', left: '700px', bottom: '170px'}}>
        <PdfCard title='Oslobodjeno poreza'/>
      </Box>
                ...
                {paginatedData && paginatedData.length > 0 ? (
                  paginatedData.map((item: Items, index) => (
                    <TableRow key={index}>
                      ...
                      <TableCell sx={{ paddingLeft: '30px', textAlign: 'left', minWidth: 150, position: 'relative', left: '105px' }}>
                        {item.Name}
                      </TableCell>

                      <TableCell sx={{ paddingRight: '40px', textAlign: 'right', minWidth: 100, position: 'relative', left: '65px' }}>
                        {item.Value}
                      </TableCell>
                      ...
                      <TableCell sx={{ paddingRight: '40px', textAlign: 'right', minWidth: 100, position: 'relative', left: '75px' }}>
                        {item.Percentage}
                      </TableCell>

                      <TableCell sx={{ paddingRight: '40px', textAlign: 'right', minWidth: 150, position: 'relative', left: '100px' }}>
                        {item.TotalFormatted}
                      </TableCell>
